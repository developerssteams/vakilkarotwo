import React from "react";
import { Link } from "react-router-dom";

function HeroSection() {
  return (
    <div>
      <section
        className="hero-section"
        style={{ backgroundImage: "url('assets/img/bg-img/1.jpg')" }}
      >
        <div className="divider"></div>

        {/* Left Side Text */}
        <div
          className="left-side-text wow fadeIn"
          data-wow-duration="1000ms"
          data-wow-delay="1200ms"
        ></div>

        {/* Right Side Image */}
        <div
          className="right-side-image wow fadeIn"
          data-wow-duration="1000ms"
          data-wow-delay="800ms"
        >
          {/* <img src="assets/img/bg-img/1.png" alt="" /> */}
        </div>

        <div className="container">
          <div className="row">
            <div className="col-12 col-md-9 col-lg-7 col-xxl-6">
              {/* Hero Content */}
              <div className="hero-content">
                <h2
                  className="mb-4 wow fadeInUp text-white"
                  data-wow-duration="1000ms"
                  data-wow-delay="400ms"
                >
                 Legal mein kuch bhi karo to, Vakilkaro
                </h2>
                <p
                  className="text-white wow fadeInUp"
                  data-wow-duration="1000ms"
                  data-wow-delay="600ms"
                >
                  Business Registration, Trademark, Taxation, Legal Compliances & Legal Services in few clicks.
                </p>
                <div className="d-flex flex-wrap align-items-center gap-4">
                  {/* Button */}
                  
                <Link to="/ContactOne" className="btn btn-primary wow fadeInUp" data-wow-duration="1000ms" data-wow-delay="800ms">
                  <span>
                    Free Consultation <i className="ti ti-arrow-up-right"></i>
                  </span>
                  <span>
                    Free Consultation <i className="ti ti-arrow-up-right"></i>
                  </span>
                </Link>

                  {/* Hero Rating */}
                  <div
                    className="hero-rating d-flex align-items-center gap-3 wow fadeInUp"
                    data-wow-duration="1000ms"
                    data-wow-delay="1000ms"
                  >
                    <div className="icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="28"
                        height="26"
                        viewBox="0 0 28 26"
                        fill="none"
                      >
                        <path
                          d="M13.0489 0.927047C13.3483 0.00573683 14.6517 0.00573993 14.9511 0.927051L17.3677 8.36475C17.5016 8.77677 17.8855 9.05573 18.3188 9.05573H26.1392C27.1079 9.05573 27.5107 10.2953 26.727 10.8647L20.4001 15.4615C20.0496 15.7161 19.903 16.1675 20.0369 16.5795L22.4535 24.0172C22.7529 24.9385 21.6984 25.7047 20.9147 25.1353L14.5878 20.5385C14.2373 20.2839 13.7627 20.2839 13.4122 20.5385L7.08533 25.1353C6.30162 25.7047 5.24714 24.9385 5.54649 24.0172L7.96315 16.5795C8.09702 16.1675 7.95036 15.7161 7.59987 15.4615L1.27299 10.8647C0.489277 10.2953 0.892056 9.05573 1.86078 9.05573H9.68123C10.1145 9.05573 10.4984 8.77677 10.6323 8.36475L13.0489 0.927047Z"
                          fill="#E8BF96"
                        />
                      </svg>
                    </div>
                    <div>
                      <p className="mb-0">5 Star</p>
                      <p className="mb-0">Read Our Success Story's</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className="divider"></div>
      </section>
    </div>
  );
}

export default HeroSection;
